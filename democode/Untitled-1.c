...
node *create_list()
{
        ...
        for(int i = 0; i < n; i++)
	{
        ...
        strcpy(new->data.name,p_cus.name);
        strcpy(new->data.address,p_cus.address);
        new->data.id=p_cus.id;
        new->data.pass=p_cus.pass;
        new->data.money=p_cus.money;
